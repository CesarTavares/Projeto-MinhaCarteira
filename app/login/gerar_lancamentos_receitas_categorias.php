...
$lancamentos_categoria = filter_input(INPUT_GET, 'categoria', FILTER_SANITIZE_SPECIAL_CHARS);
...
// var_dump($lancamentos_categoria, $data_inicio, $data_final);
if ($lancamentos_categoria && $data_inicio && $data_final && validarData($data_inicio) && validarData($data_final)) {
    try {       
        //QUERY para recuperar os registros do banco de dados
        $sql_lancamentos_cat = "SELECT des.descricao, valor, data_credito, cat.descricao as descricao_categoria, car.descricao as descricao_carteira, situacao
                                    ...
                                    WHERE des.categoria = :categoria  AND data_credito BETWEEN :data_inicio AND :data_final";
        //preparar a QUERY
        $result_categorias = $conexao->prepare($sql_lancamentos_cat);
        $result_categorias->bindParam(':categoria', $lancamentos_categoria);
        $result_categorias->bindParam(':data_inicio', $data_inicio);
        $result_categorias->bindParam(':data_final', $data_final);
        try {
        //Executar a QUERY
        ...
        $dados .= "<title>Minha Carteira - Relatorio de Lançamentos de Receitas por Categorias</title>";
        $dados .= "</head>";
        ...
        $dados .= "<h1>Relatorio de Lançamentos de Receitas por Categorias</h1>";
        $dados .= "</div>";
        ...
        //Gerar PDF
        $dompdf->stream();
        //print_r($lancamentos_categoria);

        } catch (PDOException $e) {
            echo "Erro: " . $e->getMessage();
        }
} else {
    echo "Categoria e Data Inválidas.";
}
?>