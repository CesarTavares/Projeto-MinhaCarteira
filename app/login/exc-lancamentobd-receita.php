<?php
    ...
    if($_SERVER['REQUEST_METHOD'] == 'POST'){
        ...
        $sql = "DELETE FROM `lancamentos_receitas`
                WHERE `codigo` = :codigo";
                ...
                if ($comandoSQL->rowCount() == 1) {
                    ...
                    header("Location: ./view-lancamento.php?status=excsucesso-receita");
                    exit();
                } else {
                    // Redirecionamento para altinsucesso se não houver alterações
                    header("Location: ./view-lancamento.php?status=excinsucesso-receita");
                    exit();
                }
