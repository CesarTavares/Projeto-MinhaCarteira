...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    try {
        ...
        if ($resultadoExistente['total'] > 0) {
            header('Location: ./cad-login.php?erro=lancamento-igual&nome=' . $nome . '&email=' . $email . '&senha1=' . $senha1 . '&senha2=' . $senha2 . '&nivel=' . $nivel . '&status=' . $status);
            exit();
        }
        ...
        if ($resultado) {
            ...
            header("Location: ../../index.php?status=insucesso");
            exit();
        }
